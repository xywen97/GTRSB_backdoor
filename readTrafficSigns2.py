...
def readTrafficSigns_train(rootpath):
    ...
    for c in range(33,36):
        ...
        for row in gtReader:
            ...
            if row[7] == '33':
                labels.append('0') # the 8th column is the label
            elif row[7] == '34':
                labels.append('1') # the 8th column is the label
            elif row[7] == '35':
                labels.append('2') # the 8th column is the label
        gtFile.close()
...
def readTrafficSigns_test(rootpath):
    ...
    next(gtReader) 
    for row in gtReader:
        if int(row[7]) == 33 or int(row[7]) == 34 or int(row[7]) == 35:
            images.append(plt.imread(rootpath + '/' + row[0]))
            # labels.append(str(int(row[7]-33)))
            if row[7] == '33':
                labels.append('0') # the 8th column is the label
            elif row[7] == '34':
                labels.append('1') # the 8th column is the label
